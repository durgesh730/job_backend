 ...
 * @returns {Promise<UserMangement>} - The created inventoryAuth user object.
...
const createAccount = async (body) => {
    ...
    if (await UserMangement.isEmailTaken(data.email)) {
        throw new ErrorResponse("Email Already Taken", 400);
    }

    if (await UserMangement.isTelephoneTaken(data.telephone)) {
        throw new ErrorResponse("Phone Number Already Taken", 400);
    }

    const user = await UserMangement.create(data);
    return user
}
...
const updateProfile = async (profile, userId) => {
    const updatePro = await UserMangement.findByIdAndUpdate(
